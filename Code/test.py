...
from options import *
from datasets import LocalDataset, LocalImplicitDataset
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        if "implicit" in opt['mode']:
            ...
            recovered = model(coord)
            p = PSNR(recovered, item)
            print("Compressed PSNR (dB): %0.02f" % (p))
        else:
